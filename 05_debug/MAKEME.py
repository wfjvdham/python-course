...
def routes(departing_city, destination_city):
    ...
    def build_routes(route):
        current_city = route[route.length - 1]
...
def route_length_in_kilometers(route):
    if len(route) < 2:
        ...
        for index, route in enumerate(route):
            ...
            total_length = total_length_in_km + distance_between_stations_in_meters(current_station, next_station)
            total_length = meters_to_kilometers(total_length_in_km)
...
def route_cost_in_euros(route):
    ...
    route_length_in_kilometers(route) * price_per_kilometers
...
print("Our route: %s is %d2 km costing %d2 euros" % (shortest_amsterdam_arnhem_route,
